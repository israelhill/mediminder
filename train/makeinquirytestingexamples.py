...
def dosage_daily_questions(drug):
    daily = []
    daily.append("How many pills am I taking?")
    ...
    # weekly 
def dosage_weekly_questions(drug):
    weekly = []
    weekly.append("how many %s pills am i taking per week?")
    weekly.append("how much %s am i taking weekly?")
    weekly.append("what is my weekly intake of %s?")
    weekly.append("am I taking less than 4 pills a week?")
    weekly.append("am i taking more than 5 pills a week?")
    return weekly
    
    # total
def dosage_total_questions(drug):
    total = []
    total.append("how many total %s am I using?" % drug)
    return total
...
def frequency_dotw_questions(drug):
    freq_dotw = []
    freq_dotw.append("when do I take %s" % drug)
    freq_dotw.append("on what days am i taking %s" % drug)
    freq_dotw.append("when's the next time I take %s" % drug)
    return freq_dotw
...
# pill count 
def pill_current_count_questions(drug):
    current = []
    current.append("How many %s pill do I have left?" % drug)
    current.append("How many %ss do i have right now?" % drug)
    current.append("Current count of %s pills" % drug)
    return current

def pill_run_out_questions(drug):
    run_out = []
    run_out.append("when will i run out of %s pills" % drug)
    run_out.append("how long until i finish my %s pills" % drug)
    return run_out
    # current count 
    # when will i run out 
...
def calendar_next_day_questions(drug):
    next_day = []
    next_day.append("When is the next day I'll be taking %s" % drug)
    next_day.append("Will i take %s tomorrow?" % drug)
    next_day.append("whens the next time for %s" % drug)
    return next_day

def calendar_previous_day_questions(drug):
    last_day = []
    last_day.append("When is the last day I'll be taking %s" % drug)
    last_day.append("Did i take %s yesterday" % drug)
    last_day.append("when was the last time for %s" % drug)
    return last_day
...
with open("inquirytraining.csv", 'wb') as outfile:
        ...
        for item in sample:
            ...
            write_questions(dosage_weekly_questions(item), writer, "dosage-weekly")
            write_questions(dosage_total_questions(item), writer, "dosage-total")
            write_questions(frequency_number_questions(item), writer, "frequency-number")
            write_questions(frequency_dotw_questions(item), writer, "frequency-dotw")
            ...
            write_questions(pill_current_count_questions(item), writer, "pill-current")
            write_questions(pill_run_out_questions(item), writer, "pill-run-out")
            ...
            write_questions(calendar_next_day_questions(item), writer, "calendar-next-day")
            write_questions(calendar_previous_day_questions(item), writer, "calendar-previous-day")

natural_language_classifier = NaturalLanguageClassifierV1(
    username = "2c85123b-21d8-4bde-9f35-6c66301ecbf4",
    password = "ObBtjTUN7PfP")

with open('inquirytraining.csv', 'rb') as training_data:
